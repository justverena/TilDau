...
class ProgressServiceTest {
    ...
    void validateExerciseAccess_Fails_WhenNoCourseAccess() {
        ...
        RuntimeException ex = assertThrows(RuntimeException.class, () -> progressService.validateExerciseAccess(user, exercise));

        assertEquals("User has no access to this course", ex.getMessage());
    }
    ...
    void validateExerciseAccess_Fails_WhenNoUnitAccess() {
        ...
        assertEquals("User has no access to this unit", ex.getMessage());
    }
    ...
    void validateExerciseAccess_Fails_WhenUnitCompleted() {
        ...
        when(userCourseProgressRepository.findByUserIdAndCourseId(user.getId(), course.getId()))
                .thenReturn(Optional.of(courseProgress));
        when(userUnitProgressRepository.findByUserIdAndUnitId(user.getId(), unit.getId()))
                .thenReturn(Optional.of(unitProgress));
